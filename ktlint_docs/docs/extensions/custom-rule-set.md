...
In a nutshell: a "rule set" is a JAR containing one or more [Rule](/com/pinterest/ktlint/core/Rule.kt)s gathered together in a [RuleSet](/com/pinterest/ktlint/core/RuleSet.kt). `ktlint` is relying on the
...
containing a fully qualified name of your [RuleSetProvider](/com/pinterest/ktlint/core/RuleSetProvider.kt) implementation).

## ktlint-ruleset-template

A complete sample project (with tests and build files) is included in this repo under the [ktlint-ruleset-template](ktlint-ruleset-template) directory (make sure to check [NoVarRuleTest](/yourpkgname/NoVarRuleTest.kt) as it contains some useful information).
...
While writing/debugging [Rule](/com/pinterest/ktlint/core/Rule.kt)s it's often helpful to have an AST
